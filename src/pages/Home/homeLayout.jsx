...
function HomeLayout() {
    ...
    const { books,search,setSearch } = useContext(LibraryContext);

    const {  refetch,isLoading,isError } = useGetData(search);
  ...
  return (
    <>
      ...
      <section className="min-h-[30rem] relative flex items-center justify-center bg-hero-pattern bg-cover bg-[50%] bg-no-repeat z-0">
        ...
        <h1 className="text-base-100 w-full font-semibold ">Your personal library, always with you... </h1>
        <p className="text-base-100">"Kitaplar dünyayı keşfetmenin en iyi yoludur ve okuma, zihninizi keşfetmenin en iyi yoludur." <span className="italic">- Ann Morgan</span></p>
        <form onSubmit={handleSubmit}>
        <div className="input-group flex justify-center mt-2">
    <input type="text" placeholder="Search…" value={search} onChange={(e)=>setSearch(e.target.value)}  id="floating_filled" className="input text-main  input-bordered" />
    <button type="submit" className="btn btn-square bg-main active:bg-transparent ">
      <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round"  d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
    </button>
  </div>
</form>
</div>
